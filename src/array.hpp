...
struct m_array
{
	...
	void     resize( uint32_t sSize )
	{
		...
		if ( sSize > 0 && sSize > aCapacity )
		{
			...
			if ( newData == nullptr )
				printf( "Failed to Resize m_array< %s >: %u bytes\n", typeid( T ).name(), sSize );
	...
	void reserve( uint32_t sSize )
	{
		if ( sSize > 0 && sSize > aCapacity )
		{
			void* newData = realloc( apData, sSize * sizeof( T ) );
			if ( newData == nullptr )
				printf( "Failed to Resize m_array< %s >: %zd bytes\n", typeid( T ).name(), sSize * sizeof( T ) );
	...
	void erase( const T& sItem )
	{
		...
		if ( index == UINT32_MAX )
		{
			printf( "Failed to find item in m_array< %s >!\n", typeid( T ).name() );
			return;
		}
	...
	// copying
	void assign( const m_array& other )
	{
		if ( !other.aSize )
			return;
	...
	m_array& operator=( m_array&& other )
	{
		if ( valid() )
		{
			aSize = other.aSize;
